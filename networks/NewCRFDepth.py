...
class NewCRFDepth_v2(nn.Module):
    """
    Depth network based on neural window FC-CRFs architecture.
    """
    def __init__(self, version=None, inv_depth=False, pretrained=None, 
        ...
        self.crf2 = NewCRF(input_dim=in_channels[2], embed_dim=crf_dims[2], window_size=win, v_dim=v_dims[2], num_heads=16)
        self.crf1 = NewCRF(input_dim=in_channels[1], embed_dim=crf_dims[1], window_size=win, v_dim=v_dims[1], num_heads=8)  
        self.crf0 = NewCRF(input_dim=in_channels[0], embed_dim=crf_dims[0], window_size=win, v_dim=v_dims[0], num_heads=4)   #[input_dim=192, embed_dim=128, v_dims=64]
        #in_channels = [192, 384, 768, 1536]

        self.skip_conv = nn.Sequential(
            nn.Conv2d(in_channels= 512+64, out_channels=64, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(64),
            nn.LeakyReLU()
        )
    ...
    def forward(self, imgs):
        ...
        ppm_out = self.decoder(feats) #torch.Size([1, 512, 15, 20])
        e3_skip = ppm_out #torch.Size([1, 512, 15, 20])

        e3 = self.crf3(feats[3], ppm_out)   #torch.Size([1, 1024, 15, 20])
        e3 = nn.PixelShuffle(2)(e3)  #torch.Size([1, 256, 30, 40])
        # e3_skip = e3 #torch.Size([1, 256, 30, 40])

        e2 = self.crf2(feats[2], e3) #torch.Size([1, 512, 30, 40])
        e2 = nn.PixelShuffle(2)(e2) #torch.Size([1, 128, 60, 80])
        
        e1 = self.crf1(feats[1], e2) #torch.Size([1, 256, 60, 80])
        e1 = nn.PixelShuffle(2)(e1)  #torch.Size([1, 64, 120, 160])
        e1_skip = e1 #torch.Size([1, 64, 120, 160])

        e1 = self.skip_conv(torch.cat((F.interpolate(e3_skip,size=(e1_skip.shape[2:])), e1_skip),dim=1))

        e0 = self.crf0(feats[0], e1)  #torch.Size([1, 128, 120, 160])     #[input_dim=192, embed_dim=128, v_dims=64]
